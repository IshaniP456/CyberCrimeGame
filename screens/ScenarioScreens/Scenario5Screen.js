// screens/ScenarioScreens/Scenario5Screen.js
...
const NOTES_KEY = 'scenario5_notes_v1';
const CORRECT_NAME = 'Britney Nolan';

// --- helper to keep the hero image auto-scaled but with your same look
...
export default function Scenario5Screen() {
  const insets = useSafeAreaInsets();
  const [notes, setNotes] = useState('');
  const [saving, setSaving] = useState(false);

  // guess UI (identical to Scenario #1)
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        <Text style={styles.kicker}>Scenario #5</Text>
        <Text style={styles.title}>Someone creates a fake website with pop-up tabs + sketchy ads</Text>

        <HeroImage source={require('../../assets/scenarios/fakewebsite.png')} />

        {/* Description */}
        <View style={styles.card}>
          <Text style={styles.sectionTitle}>Description</Text>
          <Text style={styles.body}>
            An individual was browsing for free sheet music for their favorite songs. They found a website that
            claimed to have downloadable piano arrangements, with flashy titles like “FREE Piano Sheets (Limited
            Access!)”. Once they clicked, the website immediately opened multiple pop-up tabs with ads promising
            “free iPhones” and “fan giveaways.” While they were distracted by closing the pop-ups, a hidden script
            ran in the background, prompting a fake CAPTCHA to "verify you’re human.” When they clicked it, the site
            auto-downloaded malware that started collecting stored browser passwords and autofill data.
          </Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.body}>Figure out who created the fake site and planted the malware.</Text>
        </View>

        {/* Clues */}
        <View style={styles.card}>
          <Text style={styles.sectionTitle}>Clues</Text>
          <Bullet>The website ends with a website URL that ends in “brrrr014.xyz”.</Bullet>
          <Bullet>IT investigates the fake website’s source code and finds it was built with a very niche code for pop-up animations.</Bullet>
          <Bullet>The site used a rare JavaScript animation library (PopFlex.js).</Bullet>
        </View>
...
const styles = StyleSheet.create({
  ...
  input: {
    minHeight: 120, borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 10,
